...
void FM77AV::IOWrite(const CanAccessMemory *accessFrom,uint16_t ioAddr,uint8_t value)
{
	...
	case FM77AVIO_SUBSYS_BUSY_HALT: // 0xFD05
		...
		else
		{
			...
			if(subCmd<FM7_MAX_SUB_CMD && 0!=(var.breakOnSubCmd[subCmd]&(MC6809::Debugger::BRKPNT_FLAG_BREAK|MC6809::Debugger::BRKPNT_FLAG_MONITOR_ONLY)))
			{
				std::string str;
				str="Sub-CPU Command";
				str+=SubCmdToStr(subCmd);
				if(0!=(var.breakOnSubCmd[subCmd]&MC6809::Debugger::BRKPNT_FLAG_BREAK))
				{
	...
	case FM77AVIO_IRQ_BEEP: // =                0xFD03,
		{
			...
			if(0!=(state.main.irqSource&state.main.irqEnableBits&~0x0f))
			{
